...
class  metrics_graph():
        ...
        for goal,paths in pa_zong.items():
            ...
            if addpath!=[] or removepath!=[]:
                if self.type == 'add':
                    ...
                    edges_index_1 = self.reverse_edge(edges_index_1)
                    edges_index_2 = self.reverse_edge(edges_index_2)
                    # 模型forward_pre参数
                    node_count = x.shape[0]
                    in_deg_1 = np.zeros(shape=node_count, dtype=np.int32)
                    out_deg_1 = np.zeros(shape=node_count, dtype=np.int32)
                    in_deg_2 = np.zeros(shape=node_count, dtype=np.int32)
                    out_deg_2 = np.zeros(shape=node_count, dtype=np.int32)
                    for r in edges_index_1[0]:
                        out_deg_1[r] += 1
                    for r in edges_index_1[1]:
                        in_deg_1[r] += 1
                    for r in edges_index_2[0]:
                        out_deg_2[r] += 1
                    for r in edges_index_2[1]:
                        in_deg_2[r] += 1
                    in_deg_1 = ms.Tensor(in_deg_1, ms.int32)
                    out_deg_1 = ms.Tensor(out_deg_1, ms.int32)
                    in_deg_2 = ms.Tensor(in_deg_2, ms.int32)
                    out_deg_2 = ms.Tensor(out_deg_2, ms.int32)
                    g1 = GraphField(ms.Tensor(edges_index_1[0], dtype=ms.int32),
                                   ms.Tensor(edges_index_1[1], dtype=ms.int32), int(node_count),
                                   int(edges_index_1.shape[1]))
                    g2 = GraphField(ms.Tensor(edges_index_2[0], dtype=ms.int32),
                                    ms.Tensor(edges_index_2[1], dtype=ms.int32), int(node_count),
                                    int(edges_index_2.shape[1]))
                    x = ms.Tensor(x,dtype=ms.float32)
                    H_ceshi[goal] = model_mask.forward_pre(x, in_deg_1, out_deg_1, in_deg_2, out_deg_2, g1, g2).asnumpy()[goal]
                elif self.type == 'mask':
    ...
    def reverse_edge(self, edge_index):
        # Assuming edge_index is convertible to numpy array and contains elements that can be made hashable.
        edge_index_np = np.array(edge_index, dtype=np.int32)  # Convert to numpy array for easy manipulation

        # Reverse the edges
        reversed_edges = np.flip(edge_index_np, 0)
        all_edges = np.hstack((edge_index_np, reversed_edges))

        # Attempting a new approach to remove duplicates by converting edge pairs to a hashable format
        unique_edges_set = set()
        for i in range(all_edges.shape[1]):
            # Convert each column (edge pair) to a tuple of integers
            edge_tuple = (int(all_edges[0, i]), int(all_edges[1, i]))
            unique_edges_set.add(edge_tuple)

        # Convert the set back to a numpy array
        unique_edges = np.array(list(unique_edges_set)).T
