...
def calculate_accuracy(outputs, targets):
    batch_size = x2ms_adapter.tensor_api.x2ms_size(targets, 0)
    ...
    _, pred = x2ms_adapter.tensor_api.topk(outputs, 1, 1, True)
    #转置
    pred = x2ms_adapter.tensor_api.t(pred)
    correct = x2ms_adapter.tensor_api.eq(pred, x2ms_adapter.tensor_api.view(targets, 1, -1))
    n_correct_elems = x2ms_adapter.tensor_api.x2ms_sum(x2ms_adapter.tensor_api.x2ms_float(correct)).data[0]
...
if __name__=='__main__':
    for i in range(2):
        ...
        x2ms_adapter.tensor_api.log(log, {
                'header1':'h1',
                'header2':'h2',
                })
