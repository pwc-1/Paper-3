...
class LRSchedulerStep(object):
    ...
    def step(self, step):
        for start, end, func in self.lr_phases:
            if step >= start:
                return func((step - start) / (end - start))
...
def annealing_cos(start, end, pct):
    ...
    cos_out = x2ms_adapter.tensor_api.cos(np, np.pi * pct) + 1
...
class CosineWarmupLR(lr_schedule_wrapper._LRScheduler):
    ...
    def get_lr(self):
        return [self.eta_min + (base_lr - self.eta_min) *
                (1 - x2ms_adapter.tensor_api.cos(math, math.pi * self.last_epoch / self.T_max)) / 2
