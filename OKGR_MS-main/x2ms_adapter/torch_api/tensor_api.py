...
def new_ones(obj, size, dtype=None, device=None, requires_grad=False):
    ones_tensor = mindspore.ops.ones(shape=tuple(size), dtype=dtype if dtype else obj.dtype)
    if not requires_grad:
        return mindspore.ops.stop_gradient(ones_tensor)
    return ones_tensor
@x2ms_func_decorator(mindspore.Tensor)
def new_ones_sizeisint(obj, size, dtype=None, device=None, requires_grad=False):
    ones_tensor = mindspore.ops.ones(shape=size, dtype=dtype if dtype else obj.dtype)
    if not requires_grad:
        return mindspore.ops.stop_gradient(ones_tensor)
    return ones_tensor
