...
class VectorPoolAggregationModule(nn.Cell):
    def __init__(
        ...
        self.separate_local_aggregation_layer = nn.SequentialCell(
            nn.Conv1d(num_c_in, num_c_out, kernel_size=1, group=self.total_voxels, has_bias=False),
        ...
        for cur_num_c in post_mlps:
            post_mlp_list.extend([
                nn.Conv1d(c_in, cur_num_c, kernel_size=1, has_bias=False),
                nn.BatchNorm1d(cur_num_c),
                nn.ReLU()
            ])
            c_in = cur_num_c
        self.post_mlps = nn.SequentialCell(*post_mlp_list)
        
        self.ReshapeWeight1 = nn.SequentialCell(
            nn.Conv1d(cur_num_c, 1, kernel_size=1, group=1, has_bias=False),
            nn.BatchNorm1d(1),
            nn.ReLU()
        )
        self.ReshapeWeight2 = nn.SequentialCell(
            nn.Conv1d(1, cur_num_c, kernel_size=1, group=1, has_bias=False),
            nn.BatchNorm1d(cur_num_c),
            nn.ReLU()
        )
    ...
    def vector_pool_with_voxel_query(self, xyz, xyz_batch_cnt, features, new_xyz, new_xyz_batch_cnt):
        ...
        new_features = mindspore.ops.cat((new_local_xyz, new_features), axis=-1).view(num_new_pts, -1)
    ...
    def get_dense_voxels_by_center(point_centers, max_neighbour_distance, num_voxels):
        ...
        x_grids = ops.arange(-R + R / num_voxels[0], R - R / num_voxels[0] + 1e-5, 2 * R / num_voxels[0])
        y_grids = ops.arange(-R + R / num_voxels[1], R - R / num_voxels[1] + 1e-5, 2 * R / num_voxels[1])
        z_grids = ops.arange(-R + R / num_voxels[2], R - R / num_voxels[2] + 1e-5, 2 * R / num_voxels[2])
    ...
    def vector_pool_with_local_interpolate(self, xyz, xyz_batch_cnt, features, new_xyz, new_xyz_batch_cnt, weights):
        ...
        voxel_features = self.local_interpolate_module.construct(
