...
class FarthestPointSampling(x2ms_adapter.autograd.Function):
    ...
    def construct(ctx, xyz, npoint: int) -> mindspore.Tensor:
        ...
        #print('pointbatch.FarthestPointSampling')
        B, N, _ = xyz.shape
        output = torch.IntTensor(B, npoint)
        temp = torch.FloatTensor(B, N).fill_(1e10)
        xyz=torch.tensor(xyz.asnumpy())
        out=pointnet2.farthest_point_sampling_wrapper(B, N, npoint, xyz.cuda(), temp.cuda(), output.cuda())
        
        output=mindspore.Tensor(out.cpu().numpy(),dtype=mindspore.int32)
        return output
...
class GatherOperation(x2ms_adapter.autograd.Function):
    ...
    def construct(ctx, features, idx) -> mindspore.Tensor:
        ...
        #print('pointbatch.GatherOperation')
        B, npoint = idx.shape
        ...
        # features=features.premute(0,2,1)
        out=pointnet2.gather_points_wrapper(B, C, N, npoint, features.cuda(), idx.cuda(), output.cuda())
        # output=gather_points(features,idx)
#         ctx.for_backwards = (idx, C, N)
        outpu=mindspore.Tensor(out.detach().cpu().numpy())
        return outpu
...
class ThreeNN(x2ms_adapter.autograd.Function):
    ...
    def construct(ctx, unknown, known) -> Tuple[mindspore.Tensor, mindspore.Tensor]:
        ...
        out=pointnet2.three_nn_wrapper(B, N, m, unknown.cuda(), known.cuda(), dist2.cuda(), idx.cuda())
        dist2,idx=out[:2]
        
        idx=mindspore.Tensor(idx.cpu().numpy(),mindspore.int32)
        return mindspore.ops.sqrt(mindspore.Tensor(dist2.cpu().numpy())), idx
...
class ThreeInterpolate(x2ms_adapter.autograd.Function):
    ...
    def construct(ctx, features, idx, weight) -> mindspore.Tensor:
        ...
        #print('pointbatch.ThreeInterpolate')
        B, c, m = features.shape
        n = idx.shape[1]
        # ctx.three_interpolate_for_backward = (idx, weight, m)
        output = torch.FloatTensor(B, c, n)
        features=torch.Tensor(features.asnumpy())
        idx=torch.IntTensor(idx.asnumpy())
        weight=torch.Tensor(weight.asnumpy())
        out=pointnet2.three_interpolate_wrapper(B, c, m, n, features.cuda(), idx.cuda(), weight.cuda(), output.cuda())
        
        output=mindspore.Tensor(out.cpu().numpy())
        return output
...
class GroupingOperation(x2ms_adapter.autograd.Function):
    ...
    def construct(ctx, features, idx) -> mindspore.Tensor:
        ...
        #print('pointbatch.GroupingOperation')
        B, nfeatures, nsample = idx.shape
        _, C, N = features.shape
        features=torch.tensor(features.asnumpy())
        idx=torch.tensor(idx.asnumpy()).int()
        output = torch.FloatTensor(B, C, nfeatures, nsample)
        out=pointnet2.group_points_wrapper(B, C, N, nfeatures, nsample, features.cuda(), idx.cuda(), output.cuda())
        # output=gather_points(features,idx)
...
class BallQuery(x2ms_adapter.autograd.Function):
    ...
    def construct(ctx, radius: float, nsample: int, xyz, new_xyz) -> mindspore.Tensor:
        ...
        idx=pointnet2.ball_query_wrapper(B, N, npoint, radius, nsample, new_xyz.cuda(), xyz.cuda(), idx.cuda())
        # idx=ball_query_1(xyz, new_xyz, radius, nsample)
        idx=mindspore.Tensor(idx.cpu().numpy(),dtype=mindspore.int32)
        return idx
...
class QueryAndGroup(nn.Cell):
    ...
    def construct(self, xyz, new_xyz, features = None) -> Tuple[mindspore.Tensor]:
        ...
        """
        #print('pointbatch.QueryAndGroup')
