...
def generate_labels(frame, pose):
    ...
    if annotations['name'].__len__() > 0:
        ...
        gt_boxes_lidar = x2ms_np.concatenate([
...
def convert_range_image_to_point_cloud(frame, range_images, camera_projections, range_image_top_pose, ri_index=(0, 1)):
    ...
    for c in calibrations:
        ...
        for cur_ri_index in ri_index:
            ...
            range_image_cartesian = x2ms_adapter.tensor_api.squeeze(tf, range_image_cartesian, axis=0)
            points_tensor = tf.gather_nd(range_image_cartesian,
                                         x2ms_adapter.tensor_api.where(tf, range_image_mask))
            points_NLZ_tensor = tf.gather_nd(range_image_NLZ, x2ms_adapter.tensor_api.where(tf.compat.v1, range_image_mask))
            points_intensity_tensor = tf.gather_nd(range_image_intensity, x2ms_adapter.tensor_api.where(tf.compat.v1, range_image_mask))
            points_elongation_tensor = tf.gather_nd(range_image_elongation, x2ms_adapter.tensor_api.where(tf.compat.v1, range_image_mask))
            cp = camera_projections[c.name][0]
            cp_tensor = tf.reshape(tf.convert_to_tensor(cp.data), cp.shape.dims)
            cp_points_tensor = tf.gather_nd(cp_tensor, x2ms_adapter.tensor_api.where(tf, range_image_mask))

            points_single.append(x2ms_adapter.tensor_api.numpy(points_tensor))
            cp_points_single.append(x2ms_adapter.tensor_api.numpy(cp_points_tensor))
            points_NLZ_single.append(x2ms_adapter.tensor_api.numpy(points_NLZ_tensor))
            points_intensity_single.append(x2ms_adapter.tensor_api.numpy(points_intensity_tensor))
            points_elongation_single.append(x2ms_adapter.tensor_api.numpy(points_elongation_tensor))

        points.append(x2ms_np.concatenate(points_single, axis=0))
        cp_points.append(x2ms_np.concatenate(cp_points_single, axis=0))
        points_NLZ.append(x2ms_np.concatenate(points_NLZ_single, axis=0))
        points_intensity.append(x2ms_np.concatenate(points_intensity_single, axis=0))
        points_elongation.append(x2ms_np.concatenate(points_elongation_single, axis=0))
...
def save_lidar_points(frame, cur_save_path, use_two_returns=True):
    ...
    points_all = x2ms_np.concatenate(points, axis=0)
    points_in_NLZ_flag = x2ms_np.concatenate(points_in_NLZ_flag, axis=0).reshape(-1, 1)
    points_intensity = x2ms_np.concatenate(points_intensity, axis=0).reshape(-1, 1)
    points_elongation = x2ms_np.concatenate(points_elongation, axis=0).reshape(-1, 1)

    num_points_of_each_lidar = [point.shape[0] for point in points]
    save_points = x2ms_np.concatenate([
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, use_two_returns=True, update_info_only=False):
    ...
    for cnt, data in enumerate(dataset):
        ...
        frame.ParseFromString(bytearray(x2ms_adapter.tensor_api.numpy(data)))
