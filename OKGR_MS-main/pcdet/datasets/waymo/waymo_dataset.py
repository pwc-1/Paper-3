...
class WaymoDataset(DatasetTemplate):
    ...
    def load_pred_boxes_to_dict(self, pred_boxes_path):
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            pred_boxes = x2ms_np.concatenate((box_dict['boxes_lidar'], box_dict['score'][:, np.newaxis], pred_labels[:, np.newaxis]), axis=-1)
    ...
    def get_lidar(self, sequence_name, sample_idx):
        ...
        points_all[:, 3] = x2ms_adapter.tensor_api.tanh(np, points_all[:, 3])
        return points_all
    ...
    def transform_prebox_to_current(pred_boxes3d, pose_pre, pose_cur):
        ...
        expand_bboxes = x2ms_np.concatenate([pred_boxes3d[:, :3], np.ones((pred_boxes3d.shape[0], 1))], axis=-1)

        bboxes_global = np.dot(expand_bboxes, pose_pre.T)[:, :3]
        expand_bboxes_global = x2ms_np.concatenate([bboxes_global[:, :3],np.ones((bboxes_global.shape[0], 1))], axis=-1)
        ...
        if pred_boxes3d.shape[-1] == 11:
            expand_vels = x2ms_np.concatenate([pred_boxes3d[:, 7:9], np.zeros((pred_boxes3d.shape[0], 1))], axis=-1)
    ...
    def get_sequence_data(self, info, points, sequence_name, sample_idx, sequence_cfg, load_pred_boxes=False):
        ...
        for idx, sample_idx_pre in enumerate(sample_idx_pre_list):
            ...
            expand_points_pre = x2ms_np.concatenate([points_pre[:, :3], np.ones((points_pre.shape[0], 1))], axis=-1)
            points_pre_global = np.dot(expand_points_pre, pose_pre.T)[:, :3]
            expand_points_pre_global = x2ms_np.concatenate([points_pre_global, np.ones((points_pre_global.shape[0], 1))], axis=-1)
            points_pre2cur = np.dot(expand_points_pre_global, np.linalg.inv(pose_cur.T))[:, :3]
            points_pre = x2ms_np.concatenate([points_pre2cur, points_pre[:, 3:]], axis=-1)
            if sequence_cfg.get('ONEHOT_TIMESTAMP', False):
        ...
        points = x2ms_np.concatenate([points] + points_pre_all, axis=0).astype(np.float32)
        num_points_all = np.array([num_pts_cur] + num_points_pre).astype(np.int32)
        poses = x2ms_np.concatenate(pose_all, axis=0).astype(np.float32)
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        for k in tqdm(range(0, len(infos), sampled_interval)):
            ...
            box_idxs_of_pts = x2ms_adapter.tensor_api.numpy(x2ms_adapter.tensor_api.squeeze(x2ms_adapter.tensor_api.long(roiaware_pool3d_utils.points_in_boxes_gpu(
                x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.from_numpy(points[:, 0:3]), dim=0)),
                x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.from_numpy(gt_boxes[:, 0:7]), dim=0))
            )), dim=0))
        ...
        # it will be used if you choose to use shared memory for gt sampling
        stacked_gt_points = x2ms_np.concatenate(stacked_gt_points, axis=0)
    ...
    def create_gt_database_of_single_scene(self, info_with_idx, database_save_path=None, use_sequence_data=False, used_classes=None,
        ...
        if use_sequence_data and crop_gt_with_tail:
            ...
            new_length = gt_boxes[:, 3] + x2ms_adapter.tensor_api.norm(np.linalg, latest_center - oldest_center, axis=-1)
        ...
        if use_cuda:
            box_idxs_of_pts = x2ms_adapter.tensor_api.numpy(x2ms_adapter.tensor_api.squeeze(x2ms_adapter.tensor_api.long(roiaware_pool3d_utils.points_in_boxes_gpu(
                x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.from_numpy(points[:, 0:3]), dim=0)),
                x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.from_numpy(gt_boxes_crop[:, 0:7]), dim=0))
            )), dim=0))
        else:
            box_point_mask = x2ms_adapter.tensor_api.numpy(x2ms_adapter.tensor_api.long(roiaware_pool3d_utils.points_in_boxes_cpu(
                x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.from_numpy(points[:, 0:3])),
                x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.from_numpy(gt_boxes_crop[:, 0:7]))
