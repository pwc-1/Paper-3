...
def limit_period(val, offset=0.5, period=np.pi):
    return val - x2ms_adapter.tensor_api.floor(np, val / period + offset) * period
...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def generate_waymo_type_results(self, infos, class_names, is_gt=False, fake_gt_infos=True):
        def boxes3d_kitti_fakelidar_to_lidar(boxes3d_lidar):
            ...
            return x2ms_np.concatenate([boxes3d_lidar[:, 0:3], l, w, h, -(r + np.pi / 2)], axis=-1)
        ...
        for frame_index, info in enumerate(infos):
            if is_gt:
                ...
                num_boxes = x2ms_adapter.tensor_api.x2ms_sum(box_mask)
        ...
        frame_id = x2ms_np.concatenate(frame_id).reshape(-1).astype(np.int64)
        boxes3d = x2ms_np.concatenate(boxes3d, axis=0)
        obj_type = np.array(obj_type).reshape(-1)
        score = x2ms_np.concatenate(score).reshape(-1)
        overlap_nlz = x2ms_np.concatenate(overlap_nlz).reshape(-1)
        difficulty = x2ms_np.concatenate(difficulty).reshape(-1).astype(np.int8)
    ...
    def mask_by_distance(self, distance_thresh, boxes_3d, *args):
        mask = x2ms_adapter.tensor_api.norm(np.linalg, boxes_3d[:, 0:2], axis=1) < distance_thresh + 0.5
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True):
        ...
        print('Level 1: %d, Level2: %d)' % (x2ms_adapter.tensor_api.x2ms_sum((gt_difficulty == 1)), x2ms_adapter.tensor_api.x2ms_sum((gt_difficulty == 2))))

        if x2ms_adapter.tensor_api.x2ms_max(pd_score) > 1:
            # assert pd_score.max() <= 1.0, 'Waymo evaluation only supports normalized scores'
            pd_score = 1 / (1 + x2ms_adapter.tensor_api.exp(np, -pd_score))
