...
class PointFeatureEncoder(object):
    ...
    def construct(self, data_dict):
        ...
        if self.point_encoding_config.get('filter_sweeps', False) and 'timestamp' in self.src_feature_list:
            ...
            dt = x2ms_adapter.tensor_api.x2ms_round(np, data_dict['points'][:, idx], 2)
            max_dt = sorted(x2ms_adapter.tensor_api.unique(np, dt))[min(len(x2ms_adapter.tensor_api.unique(np, dt))-1, max_sweeps-1)]
    ...
    def absolute_coordinates_encoding(self, points=None):
        ...
        point_features = x2ms_np.concatenate(point_feature_list, axis=1)
