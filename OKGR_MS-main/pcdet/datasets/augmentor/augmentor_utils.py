...
def global_frustum_dropout_top(gt_boxes, points, intensity_range):
    ...
    threshold = x2ms_adapter.tensor_api.x2ms_max(np, points[:, 2]) - intensity * (x2ms_adapter.tensor_api.x2ms_max(np, points[:, 2]) - x2ms_adapter.tensor_api.x2ms_min(np, points[:, 2]))
...
def global_frustum_dropout_bottom(gt_boxes, points, intensity_range):
    ...
    threshold = x2ms_adapter.tensor_api.x2ms_min(np, points[:, 2]) + intensity * (x2ms_adapter.tensor_api.x2ms_max(np, points[:, 2]) - x2ms_adapter.tensor_api.x2ms_min(np, points[:, 2]))
    points = points[points[:, 2] > threshold]
    gt_boxes = gt_boxes[gt_boxes[:, 2] > threshold]
...
def global_frustum_dropout_left(gt_boxes, points, intensity_range):
    ...
    threshold = x2ms_adapter.tensor_api.x2ms_max(np, points[:, 1]) - intensity * (x2ms_adapter.tensor_api.x2ms_max(np, points[:, 1]) - x2ms_adapter.tensor_api.x2ms_min(np, points[:, 1]))
...
def global_frustum_dropout_right(gt_boxes, points, intensity_range):
    ...
    threshold = x2ms_adapter.tensor_api.x2ms_min(np, points[:, 1]) + intensity * (x2ms_adapter.tensor_api.x2ms_max(np, points[:, 1]) - x2ms_adapter.tensor_api.x2ms_min(np, points[:, 1]))
    points = points[points[:, 1] > threshold]
    gt_boxes = gt_boxes[gt_boxes[:, 1] > threshold]
...
def get_points_in_box(points, gt_box):
    ...
    MARGIN = 1e-1
    cosa, sina = x2ms_adapter.tensor_api.cos(math, -rz), x2ms_adapter.tensor_api.sin(math, -rz)
...
def get_pyramids(boxes):
    ...
    pyramids = x2ms_np.concatenate(pyramid_list, axis=1)  # [N, 6, 15], 15=5*3
    return pyramids
...
def local_pyramid_dropout(gt_boxes, points, dropout_prob, pyramids=None):
    ...
    if x2ms_adapter.tensor_api.x2ms_sum(np, drop_box_mask) != 0:
        ...
        points = points[np.logical_not(x2ms_adapter.tensor_api.x2ms_any(point_masks, -1))]
    # print(drop_box_mask)
...
def local_pyramid_sparsify(gt_boxes, points, prob, max_num_pts, pyramids=None):
    ...
    if pyramids.shape[0] > 0:
        ...
        pyramid_sampled_points_num = x2ms_adapter.tensor_api.x2ms_sum(pyramid_sampled_point_masks, 0)  # the number of points in each surface pyramid
        ...
        if sparsify_pyramids.shape[0] > 0:
            ...
            remain_points = points[
                np.logical_not(x2ms_adapter.tensor_api.x2ms_any(point_masks, -1))]  # points which outside the down sampling pyramid
            ...
            sparsified_points = x2ms_np.concatenate(sparsified_points, axis=0)
            points = x2ms_np.concatenate([remain_points, sparsified_points], axis=0)
...
def local_pyramid_swap(gt_boxes, points, prob, max_num_pts, pyramids=None):
    def get_points_ratio(points, pyramid):
        ...
        alphas = x2ms_adapter.tensor_api.x2ms_sum(((points[:, 0:3] - pyramid[3:6]) * vector_0), -1) / x2ms_adapter.tensor_api.x2ms_sum(np.power(vector_0, 2))
        betas = x2ms_adapter.tensor_api.x2ms_sum(((points[:, 0:3] - pyramid[3:6]) * vector_1), -1) / x2ms_adapter.tensor_api.x2ms_sum(np.power(vector_1, 2))
        gammas = x2ms_adapter.tensor_api.x2ms_sum(((points[:, 0:3] - surface_center) * vector_2), -1) / x2ms_adapter.tensor_api.x2ms_sum(np.power(vector_2, 2))
    ...
    if x2ms_adapter.tensor_api.x2ms_sum(swap_pyramid_mask) > 0:
        point_masks = points_in_pyramids_mask(points, pyramids)
        point_nums = x2ms_adapter.tensor_api.x2ms_sum(point_masks, 0).reshape(pyramids.shape[0], -1)  # [N, 6]
        ...
        if x2ms_adapter.tensor_api.x2ms_sum(selected_pyramids) > 0:
            # get to_swap pyramids
            index_i, index_j = x2ms_adapter.tensor_api.nonzero(np, selected_pyramids)
            selected_pyramid_indices = [np.random.choice(index_j[index_i == i]) \
                                            if e and x2ms_adapter.tensor_api.x2ms_any((index_i == i)) else 0 for i, e in
            ...
            index_i, index_j = x2ms_adapter.tensor_api.nonzero(np, selected_pyramids_mask)
            non_zero_pyramids_mask[selected_pyramids_mask] = False
            swapped_index_i = np.array([np.random.choice(x2ms_adapter.tensor_api.where(np, non_zero_pyramids_mask[:, j])[0]) if \
                                            x2ms_adapter.tensor_api.where(np, non_zero_pyramids_mask[:, j])[0].shape[0] > 0 else
                                        index_i[i] for i, j in enumerate(x2ms_adapter.tensor_api.tolist(index_j))])
            swapped_indicies = x2ms_np.concatenate([swapped_index_i[:, None], index_j[:, None]], axis=1)
            ...
            swap_pyramids = x2ms_np.concatenate([to_swap_pyramids, swapped_pyramids], axis=0)
            swap_point_masks = points_in_pyramids_mask(points, swap_pyramids)
            remain_points = points[np.logical_not(x2ms_adapter.tensor_api.x2ms_any(swap_point_masks, -1))]
            ...
            for i in range(num_swapped_pyramids):
                ...
                to_swap_points_intensity_ratio = (to_swap_points[:, -1:] - x2ms_adapter.tensor_api.x2ms_min(to_swap_points[:, -1:])) / \
                                                 np.clip(
                                                     (x2ms_adapter.tensor_api.x2ms_max(to_swap_points[:, -1:]) - x2ms_adapter.tensor_api.x2ms_min(to_swap_points[:, -1:])),
                                                     1e-6, 1)
                swapped_points_intensity_ratio = (swapped_points[:, -1:] - x2ms_adapter.tensor_api.x2ms_min(swapped_points[:, -1:])) / \
                                                 np.clip(
                                                     (x2ms_adapter.tensor_api.x2ms_max(swapped_points[:, -1:]) - x2ms_adapter.tensor_api.x2ms_min(swapped_points[:, -1:])),
                                                     1e-6, 1)
                ...
                new_to_swap_points_intensity = recover_points_intensity_by_ratio(
                    swapped_points_intensity_ratio, x2ms_adapter.tensor_api.x2ms_max(to_swap_points[:, -1:]),
                    x2ms_adapter.tensor_api.x2ms_min(to_swap_points[:, -1:]))
                new_swapped_points_intensity = recover_points_intensity_by_ratio(
                    to_swap_points_intensity_ratio, x2ms_adapter.tensor_api.x2ms_max(swapped_points[:, -1:]),
                    x2ms_adapter.tensor_api.x2ms_min(swapped_points[:, -1:]))
                ...
                new_to_swap_points = x2ms_np.concatenate([new_to_swap_points, new_to_swap_points_intensity], axis=1)
                new_swapped_points = x2ms_np.concatenate([new_swapped_points, new_swapped_points_intensity], axis=1)
                
                points_res.append(new_to_swap_points)
                points_res.append(new_swapped_points)
            
            points_res = x2ms_np.concatenate(points_res, axis=0)
            points = x2ms_np.concatenate([remain_points, points_res], axis=0)
    return gt_boxes, points
