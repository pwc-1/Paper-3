...
def sample_points_with_roi(rois, points, points_xyzr, sample_radius_with_roi, num_max_points_of_part=200000):
    ...
    if points.shape[0] < num_max_points_of_part:
        ...
        t=points[:, None, :] - rois[None, :, 0:3]
        distance = torch.tensor(t.asnumpy()).norm(dim=-1)
        min_dis, min_dis_roi_idx = distance.min(dim=-1)
        rois=torch.tensor(rois.asnumpy())
    ...
    s=point_mask.sum()
    point_mask=mindspore.Tensor(point_mask.numpy())
    sampled_points = points[:1] if s == 0 else points[point_mask, :]
    if points_xyzr != None:
            ...
            if potential_occluded_points_list != None and weights == None:
                for bs_idx in range(batch_size):
                    ...
                    if potential_occluded_points_list[bs_idx] != [0]:
                        ...
                        xyz_batch_cnt[bs_idx] = x2ms_adapter.tensor_api.x2ms_sum(valid_mask)+potential_occluded_points_list[bs_idx].shape[0]
                    else:
                ...
                for i in range(roi_local_label[-1] + 1): # 以roi_local_label最后一个数 -- 去除false postive的roi的个数, 循环每一个roi
                    ...
                    if len(occluded_points_one_box) != 0 and len(occluded_points_one_box) != 1:
                        ...
                        if fg_of_rois_b_selected!=None:
                            ...
                            whole_object_kp = ops.cat(occluded_points_one_box)[:, 1:]
                            bs_whole_object_kp = ops.cat(occluded_points_one_box)[:, 0]
                            f_whole_object_kp = ops.cat(occluded_points_one_box)[:, 4]
                        ...
                        for o0 in range(occ.shape[0]):
                            ...
                            if roi_label == 2 or roi_label == 3:  # if roi_dz > np.max([roi_dx, roi_dy]):
                                ...
                                distance2points_score_one_box.append(torch.Tensor(l1.asnumpy()))
                                ...
                                distance2points_score_one_box.append(torch.Tensor(l2.asnumpy()))
                        ...
                        if len([n for n in if_in_cylinder_mask if n == True]) != 0:
                            ...
                            distance2points_score_one_box_torch=mindspore.tensor(distance2points_score_one_box_torch.numpy())
                            proposal2points_score_one_box_torch=mindspore.tensor(proposal2points_score_one_box_torch.numpy())
