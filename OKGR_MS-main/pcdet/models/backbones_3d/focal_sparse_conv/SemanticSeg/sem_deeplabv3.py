...
from x2ms_adapter import load_state_dict_from_url
import mindspore
...
class SegTemplate(nn.Cell):
    def __init__(self, constructor, feat_extract_layer, num_classes, pretrained_path=None, aux_loss=None):
        ...
        if self.pretrained:
            # Preprocess Module
            self.norm_mean = x2ms_adapter.Tensor([0.485, 0.456, 0.406])
            self.norm_std = x2ms_adapter.Tensor([0.229, 0.224, 0.225])
    ...
    def get_model(self, constructor):
        ...
        if self.pretrained_path is not None:
            model_dict = x2ms_adapter.nn_cell.state_dict(model)
            ...
            if not checkpoint_path.exists():
                ...
                load_state_dict_from_url(url, save_dir)

            # Get pretrained state dict
            pretrained_dict = x2ms_adapter.load(self.pretrained_path)
            ...
            x2ms_adapter.load_state_dict(model, model_dict, strict=False)
    ...
    def construct(self, images):
        ...
        # Preprocess images
        if self.pretrained:
            images = (images - x2ms_adapter.tensor_api.type_as(self.norm_mean[None, :, None, None], images)) / x2ms_adapter.tensor_api.type_as(self.norm_std[None, :, None, None], images)
        x = images
        ...
        if self.model.aux_classifier is not None:
            ...
            x = x2ms_adapter.nn_functional.interpolate(x, size=feat_shape, mode='bilinear', align_corners=False)
            result["aux"] = x
