...
class PMPNet(nn.Cell):
    ...
    def construct(self, point_cloud):
        ...
        prev_s = {
            'l0': x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, 128, npoint), dtype=mindspore.float32, device=device) * 0.01),
            'l1': x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, 128, 512), dtype=mindspore.float32, device=device) * 0.01),
            'l2': x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, 256, 128), dtype=mindspore.float32, device=device) * 0.01)
        }
        ...
        return [x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(pcd_out_1, 0, 2, 1)), x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(pcd_out_2, 0, 2, 1))], [delta1, delta2]
