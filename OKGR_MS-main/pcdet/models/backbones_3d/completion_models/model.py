...
class Unit(nn.Cell):
    def __init__(self, step=1, in_channel=256):
        ...
        self.conv_z = Conv1d(in_channel * 2, in_channel, if_bn=True, activation_fn=x2ms_adapter.sigmoid)
        self.conv_r = Conv1d(in_channel * 2, in_channel, if_bn=True, activation_fn=x2ms_adapter.sigmoid)
        self.conv_h = Conv1d(in_channel * 2, in_channel, if_bn=True, activation_fn=mindspore.ops.relu)

    def construct(self, cur_x, prev_s):
        """
        Args:
        ...
        z = self.conv_z(x2ms_adapter.cat([cur_x, prev_s], 1))
        r = self.conv_r(x2ms_adapter.cat([cur_x, prev_s], 1))
        h_hat = self.conv_h(x2ms_adapter.cat([cur_x, r * prev_s], 1))
        h = (1 - z) * cur_x + z * h_hat
        return h, h


class StepModel(nn.Cell):
    def __init__(self, step=1):
        ...
        self.mlp_conv = x2ms_nn.Sequential(*mlp_conv)
    ...
    def construct(self, raw, centraled, prev_s):
        ...
        l2_xyz, l2_points = x2ms_adapter.cat((centraled2_xyz, raw2_xyz), 2), x2ms_adapter.cat((centraled2_points, raw2_points), 2)# (B, 3, 32), (B, 256, 32)
        ...
        centraled0_points = self.fp_module_1(centraled0_xyz, centraled1_xyz, x2ms_adapter.cat([centraled0_xyz, centraled0_points], 1), centraled1_points)
        centraled0_points, prev_s['l0'] = self.unit_1(centraled0_points, prev_s['l0'])

        b, _, n = centraled0_points.shape
        noise = x2ms_adapter.normal(mean=0.0, std=x2ms_adapter.ones((b, 32, n)))
        delta_xyz = x2ms_adapter.tanh(self.mlp_conv(x2ms_adapter.cat([centraled0_points, noise], 1))) * 1.0 / 10 ** (self.step - 1) # (B, 3, )
...
class StepModelNoise(nn.Cell):
    ...
    def construct(self, point_cloud, prev_s):
        device = point_cloud.device
        l0_xyz = point_cloud
        l0_points = point_cloud
        ...
        noise_points = x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, (self.noise_dim if self.if_noise else 0), n),
                                                           device=device) * self.noise_stdv)
        l0_points = x2ms_adapter.cat([l0_points, noise_points], 1)
        ...
        l0_points = self.fp_module_1(l0_xyz, l1_xyz, x2ms_adapter.cat([l0_xyz, l0_points], 1), l1_points)
        l0_points, prev_s['l0'] = self.unit_1(l0_points, prev_s['l0'])  # (B, 128, 2048)

        noise = x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, 32, n), device=device))
        delta_xyz = x2ms_adapter.tanh(self.mlp_conv(x2ms_adapter.cat([l0_points, noise], 1))) * 1.0 / 10 ** (self.step - 1)
        point_cloud = point_cloud + delta_xyz
        return point_cloud, delta_xyz


class PMPNet(nn.Cell):
    ...
    def construct(self, point_cloud, central_len):
        """
        Args:
        ...
        centraled, raw = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(centraled, 0, 2, 1)), x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(raw, 0, 2, 1))

        b, npoint, _ = point_cloud.shape
        # device = point_cloud.device
        # point_cloud = point_cloud.permute(0, 2, 1).contiguous()
        prev_s = {
            'l0': x2ms_adapter.normal(mean=0.0, std=x2ms_adapter.ones((b, 128, npoint), dtype=mindspore.float32) * 0.01),
            'l1': x2ms_adapter.normal(mean=0.0, std=x2ms_adapter.ones((b, 128, 512), dtype=mindspore.float32) * 0.01),
            'l2': x2ms_adapter.normal(mean=0.0, std=x2ms_adapter.ones((b, 256, 128), dtype=mindspore.float32) * 0.01)
        }
        ...
        pcd_out_1 = x2ms_adapter.cat((centraled_pcd_out1, raw_pcd_out1), 2)
        ...
        raw_pcd_out2, centraled_pcd_out2, _, delta2 = self.step_2(raw_pcd_out1, centraled_pcd_out1, prev_s)
        pcd_out_2 = x2ms_adapter.cat((centraled_pcd_out2, raw_pcd_out2), 2)
        return [x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(pcd_out_1, 0, 2, 1)), x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(pcd_out_2, 0, 2, 1))], [delta1, delta2], [x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(raw_pcd_out1, 0, 2, 1)), x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(raw_pcd_out2, 0, 2, 1))], [x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(centraled_pcd_out1, 0, 2, 1)), x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(centraled_pcd_out2, 0, 2, 1))]



class StepModelTransformer(nn.Cell):
    ...
    def construct(self, point_cloud, prev_s):
        ...
        l0_points = self.fp_module_1(l0_xyz, l1_xyz, x2ms_adapter.cat([l0_xyz, l0_xyz], 1), l1_points)
        l0_points, prev_s['l0'] = self.unit_1(l0_points, prev_s['l0'])  # (B, 128, 2048)

        noise = x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, self.dim_tail, n), device=device) * self.noise_stdv)
        l0_points = x2ms_adapter.cat([l0_points, noise], 1)
        delta_xyz = x2ms_adapter.tanh(self.mlp_conv(l0_points)) * 1.0 / 10 ** (self.step - 1)
        point_cloud = point_cloud + delta_xyz
        return point_cloud, delta_xyz

class PMPNetPlus(nn.Cell):
    ...
    def construct(self, point_cloud):
        """
        Args:
            point_cloud: Tensor, (B, 2048, 3)
        """
        b, npoint, _ = point_cloud.shape
        device = point_cloud.device
        point_cloud = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(point_cloud, 0, 2, 1))
        prev_s = {
            'l0': x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, 128, npoint), dtype=mindspore.half, device=device) * 0.01),
            'l1': x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, 128, 512), dtype=mindspore.half, device=device) * 0.01),
            'l2': x2ms_adapter.normal(mean=0, std=x2ms_adapter.ones((b, 256, 128), dtype=mindspore.half, device=device) * 0.01)
        }
        ...
        return [x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(pcd_out_1, 0, 2, 1))], [delta1]