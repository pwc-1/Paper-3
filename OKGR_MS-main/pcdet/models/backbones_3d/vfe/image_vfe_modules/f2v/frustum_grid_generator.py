...
class FrustumGridGenerator(nn.Cell):
    ...
    def __init__(self, grid_size, pc_range, disc_cfg):
        ...
        self.grid_size = x2ms_adapter.as_tensor(grid_size, dtype=self.dtype)
        self.pc_range = pc_range
        self.out_of_bounds_val = -2
        self.disc_cfg = disc_cfg

        # Calculate voxel size
        pc_range = x2ms_adapter.as_tensor(pc_range).reshape(2, 3)
        self.pc_min = pc_range[0]
        self.pc_max = pc_range[1]
        self.voxel_size = (self.pc_max - self.pc_min) / self.grid_size

        # Create voxel grid
        self.depth, self.width, self.height = x2ms_adapter.tensor_api.x2ms_int(self.grid_size)
        self.voxel_grid = create_meshgrid3d(depth=self.depth,
                                                         height=self.height,
                                                         width=self.width,
                                                         normalized_coordinates=False)

        self.voxel_grid = x2ms_adapter.tensor_api.permute(self.voxel_grid, 0, 1, 3, 2, 4)  # XZY-> XYZ
    ...
    def grid_to_lidar_unproject(self, pc_min, voxel_size):
        ...
        unproject = x2ms_adapter.x2ms_tensor([[x_size, 0, 0, x_min],
                                  [0, y_size, 0, y_min],
                                  [0,  0, z_size, z_min],
                                  [0,  0, 0, 1]],
    ...
    def transform_grid(self, voxel_grid, grid_to_lidar, lidar_to_cam, cam_to_img):
        ...
        voxel_grid = x2ms_adapter.tensor_api.repeat_interleave(voxel_grid, repeats=B, dim=0)
        ...
        image_depths = x2ms_adapter.tensor_api.unsqueeze(image_depths, -1)
        frustum_grid = x2ms_adapter.cat((image_grid, image_depths), dim=-1)
        return frustum_grid

    def construct(self, lidar_to_cam, cam_to_img, image_shape):
        """
        ...
        frustum_grid = self.transform_grid(voxel_grid=x2ms_adapter.to(self.voxel_grid, lidar_to_cam.device),
                                           grid_to_lidar=x2ms_adapter.to(self.grid_to_lidar, lidar_to_cam.device),
                                           lidar_to_cam=lidar_to_cam,
                                           cam_to_img=cam_to_img)

        # Normalize grid
        image_shape, _ = x2ms_adapter.x2ms_max(image_shape, dim=0)
        image_depth = x2ms_adapter.x2ms_tensor([self.disc_cfg["num_bins"]],
                                   device=image_shape.device,
                                   dtype=image_shape.dtype)
        frustum_shape = x2ms_adapter.cat((image_depth, image_shape))
        ...
        mask = ~x2ms_adapter.isfinite(frustum_grid)
