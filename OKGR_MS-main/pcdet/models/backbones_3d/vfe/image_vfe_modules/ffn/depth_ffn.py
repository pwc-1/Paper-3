...
class DepthFFN(nn.Cell):
    ...
    def create_frustum_features(self, image_features, depth_logits):
        ...
        image_features = x2ms_adapter.tensor_api.unsqueeze(image_features, depth_dim)
        depth_logits = x2ms_adapter.tensor_api.unsqueeze(depth_logits, channel_dim)

        # Apply softmax along depth axis and remove last depth category (> Max Range)
        depth_probs = x2ms_adapter.nn_functional.softmax(depth_logits, dim=depth_dim)
