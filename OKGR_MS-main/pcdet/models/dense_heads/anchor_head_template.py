...
class AnchorHeadTemplate(nn.Cell):
    ...
    def copy_to_parent(self,c_dict):
        self.forward_ret_dict.update(c_dict)
    ...
    def assign_targets(self, gt_boxes):
        ...
        """
        self.targets_dict=self.target_assigner.assign_targets(
    ...
    def get_cls_layer_loss(self):
        ...
        box_cls_labels = self.targets_dict['box_cls_labels']
    ...
    def get_direction_target(anchors, reg_targets, one_hot=True, dir_offset=0, num_bins=2):
        ...
        if one_hot:
            ...
            x2ms_adapter.tensor_api.scatter_(dir_targets, -1, x2ms_adapter.tensor_api.long(x2ms_adapter.tensor_api.unsqueeze(dir_cls_targets, dim=-1)), 1.0)
    ...
    def get_box_reg_layer_loss(self):
        ...
        box_reg_targets = self.targets_dict['box_reg_targets']
        ...
        if isinstance(self.anchors, list):
            if self.use_multihead:
                anchors = x2ms_adapter.cat(
                    [x2ms_adapter.tensor_api.view(x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(anchor, 3, 4, 0, 1, 2, 5)), -1, anchor.shape[-1]) for anchor in
                     self.anchors], dim=0)
            else:
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        if isinstance(self.anchors, list):
            if self.use_multihead:
                anchors = x2ms_adapter.cat([x2ms_adapter.tensor_api.view(x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(anchor, 3, 4, 0, 1, 2, 5)), -1, anchor.shape[-1])
        ...
        batch_anchors = x2ms_adapter.tensor_api.repeat(x2ms_adapter.tensor_api.view(anchors, 1, -1, anchors.shape[-1]), batch_size, 1, 1)
        ...
        if dir_cls_preds is not None:
            ...
            dir_cls_preds = x2ms_adapter.tensor_api.view(dir_cls_preds, batch_size, num_anchors, -1) if not isinstance(dir_cls_preds, list) \
            ...
            # a=mindspore.ops.unsqueeze(dir_offset+dir_rot+ period * dir_labels.to(batch_box_preds.dtype),dim=2)
            # batch_box_preds =mindspore.Tensor(batch_box_preds)
            # batch_box_preds[..., 6] =  batch_box_preds[..., 6]
            # tensor_x = mindspore.Tensor(np.arange(3 * 4).reshape(3, 4,1).astype(np.float32))
            # tensor_y = mindspore.Tensor(np.arange(3 * 4).reshape(3, 4,1).astype(np.float32))
            # tensor_x[[0, 1], 1:3,:] += 2
            # tensor_y[[1], ...] -= [4, 3, 2, 1]
