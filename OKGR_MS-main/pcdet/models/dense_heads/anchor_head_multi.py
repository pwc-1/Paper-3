...
class SingleHead(BaseBEVBackbone):
    def __init__(self, model_cfg, input_channels, num_class, num_anchors_per_location, code_size, rpn_head_cfg=None,
        ...
        x2ms_adapter.nn_cell.register_buffer(self, 'head_label_indices', head_label_indices)

        if self.separate_reg_config is not None:
            code_size_cnt = 0
            self.conv_box = x2ms_nn.ModuleDict()
            self.conv_box_names = []
            ...
            for k in range(num_middle_conv):
                conv_cls_list.extend([
                    ...
                    x2ms_nn.BatchNorm2d(num_middle_filter),
                    x2ms_nn.ReLU()
                ])
                c_in = num_middle_filter
            conv_cls_list.append(x2ms_nn.Conv2d(
                c_in, self.num_anchors_per_location * self.num_class,
                kernel_size=3, stride=1, padding=1
            ))
            self.conv_cls = x2ms_nn.Sequential(*conv_cls_list)

            for reg_config in self.separate_reg_config.REG_LIST:
                reg_name, reg_channel = x2ms_adapter.tensor_api.split(reg_config, ':')
                ...
                cur_conv_list.append(x2ms_nn.Conv2d(
                    c_in, self.num_anchors_per_location * int(reg_channel),
                    kernel_size=3, stride=1, padding=1, bias=True
                ))
                code_size_cnt += reg_channel
                self.conv_box[f'conv_{reg_name}'] = x2ms_nn.Sequential(*cur_conv_list)
                self.conv_box_names.append(f'conv_{reg_name}')

            for m in x2ms_adapter.nn_cell.modules(self.conv_box):
                if isinstance(m, x2ms_nn.Conv2d):
                    x2ms_adapter.nn_init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            ...
            self.conv_cls = x2ms_nn.Conv2d(
                input_channels, self.num_anchors_per_location * self.num_class,
                kernel_size=1
            )
            self.conv_box = x2ms_nn.Conv2d(
                input_channels, self.num_anchors_per_location * self.code_size,
                kernel_size=1
            )

        if self.model_cfg.get('USE_DIRECTION_CLASSIFIER', None) is not None:
            self.conv_dir_cls = x2ms_nn.Conv2d(
                input_channels,
    ...
    def init_weights(self):
        pi = 0.01
        if isinstance(self.conv_cls, x2ms_nn.Conv2d):
            x2ms_adapter.nn_init.constant_(self.conv_cls.bias, -x2ms_adapter.tensor_api.log(np, (1 - pi) / pi))
        else:
            x2ms_adapter.nn_init.constant_(self.conv_cls[-1].bias, -x2ms_adapter.tensor_api.log(np, (1 - pi) / pi))

    def construct(self, spatial_features_2d):
        ret_dict = {}
        spatial_features_2d = x2ms_adapter.forward(super(), {'spatial_features': spatial_features_2d})['spatial_features_2d']
        ...
        if self.separate_reg_config is None:
            ...
            box_preds = x2ms_adapter.cat(box_preds_list, dim=1)

        if not self.use_multihead:
            box_preds = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(box_preds, 0, 2, 3, 1))
            cls_preds = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(cls_preds, 0, 2, 3, 1))
        else:
            H, W = box_preds.shape[2:]
            batch_size = box_preds.shape[0]
            box_preds = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(x2ms_adapter.tensor_api.view(box_preds, -1, self.num_anchors_per_location,
                                       self.code_size, H, W), 0, 1, 3, 4, 2))
            cls_preds = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(x2ms_adapter.tensor_api.view(cls_preds, -1, self.num_anchors_per_location,
                                       self.num_class, H, W), 0, 1, 3, 4, 2))
            box_preds = x2ms_adapter.tensor_api.view(box_preds, batch_size, -1, self.code_size)
            cls_preds = x2ms_adapter.tensor_api.view(cls_preds, batch_size, -1, self.num_class)

        if self.conv_dir_cls is not None:
            dir_cls_preds = self.conv_dir_cls(spatial_features_2d)
            if self.use_multihead:
                dir_cls_preds = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(x2ms_adapter.tensor_api.view(
                    dir_cls_preds, -1, self.num_anchors_per_location, self.model_cfg.NUM_DIR_BINS, H, W), 0, 1, 3, 4,
                                                                                                  2))
                dir_cls_preds = x2ms_adapter.tensor_api.view(dir_cls_preds, batch_size, -1, self.model_cfg.NUM_DIR_BINS)
            else:
                dir_cls_preds = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(dir_cls_preds, 0, 2, 3, 1))
...
class AnchorHeadMulti(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        if self.model_cfg.get('SHARED_CONV_NUM_FILTER', None) is not None:
            ...
            self.shared_conv = x2ms_nn.Sequential(
                x2ms_nn.Conv2d(input_channels, shared_conv_num_filter, 3, stride=1, padding=1, bias=False),
                x2ms_nn.BatchNorm2d(shared_conv_num_filter, eps=1e-3, momentum=0.01),
                x2ms_nn.ReLU(),
            )
        else:
    ...
    def make_multihead(self, input_channels):
        ...
        for rpn_head_cfg in rpn_head_cfgs:
            ...
            head_label_indices = x2ms_adapter.from_numpy(np.array([
        ...
        self.rpn_heads = x2ms_nn.ModuleList(rpn_heads)
    ...
    def construct(self, data_dict):
        ...
        ret = {
            'cls_preds': cls_preds if self.separate_multihead else x2ms_adapter.cat(cls_preds, dim=1),
            'box_preds': box_preds if self.separate_multihead else x2ms_adapter.cat(box_preds, dim=1),
        }

        if self.model_cfg.get('USE_DIRECTION_CLASSIFIER', False):
            dir_cls_preds = [ret_dict['dir_cls_preds'] for ret_dict in ret_dicts]
            ret['dir_cls_preds'] = dir_cls_preds if self.separate_multihead else x2ms_adapter.cat(dir_cls_preds, dim=1)
    ...
    def get_cls_layer_loss(self):
        ...
        cls_weights = x2ms_adapter.tensor_api.x2ms_float((negative_cls_weights + pos_cls_weight * positives))

        reg_weights = x2ms_adapter.tensor_api.x2ms_float(positives)
        if self.num_class == 1:
            # class agnostic
            box_cls_labels[positives] = 1
        pos_normalizer = x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.tensor_api.x2ms_sum(positives, 1, keepdim=True))
        ...
        x2ms_adapter.tensor_api.scatter_(one_hot_targets, -1, x2ms_adapter.tensor_api.long(x2ms_adapter.tensor_api.unsqueeze(cls_targets, dim=-1)), 1.0)
        ...
        for idx, cls_pred in enumerate(cls_preds):
            ...
            cls_pred = x2ms_adapter.tensor_api.view(cls_pred, batch_size, -1, cur_num_class)
        ...
        tb_dict = {
            'rpn_loss_cls': x2ms_adapter.tensor_api.item(cls_losses)
        }
    ...
    def get_box_reg_layer_loss(self):
        ...
        if isinstance(self.anchors, list):
            if self.use_multihead:
                anchors = x2ms_adapter.cat(
                    [x2ms_adapter.tensor_api.view(x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.permute(anchor, 3, 4, 0, 1, 2, 5)), -1, anchor.shape[-1])
        ...
        anchors = x2ms_adapter.tensor_api.repeat(x2ms_adapter.tensor_api.view(anchors, 1, -1, anchors.shape[-1]), batch_size, 1, 1)

        start_idx = 0
        box_losses = 0
        tb_dict = {}
        for idx, box_pred in enumerate(box_preds):
            box_pred = x2ms_adapter.tensor_api.view(
                box_pred, batch_size, -1,
            ...
            box_losses += loc_loss
            tb_dict['rpn_loss_loc'] = tb_dict.get('rpn_loss_loc', 0) + x2ms_adapter.tensor_api.item(loc_loss)
            ...
            if box_dir_cls_preds is not None:
                ...
                dir_logit = x2ms_adapter.tensor_api.view(box_dir_cls_pred, batch_size, -1, self.model_cfg.NUM_DIR_BINS)
                weights = x2ms_adapter.tensor_api.type_as(positives, dir_logit)
                ...
                box_losses += dir_loss
                tb_dict['rpn_loss_dir'] = tb_dict.get('rpn_loss_dir', 0) + x2ms_adapter.tensor_api.item(dir_loss)
