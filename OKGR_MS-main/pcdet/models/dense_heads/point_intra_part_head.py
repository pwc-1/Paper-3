...
class PointIntraPartOffsetHead(PointHeadTemplate):
    ...
    def assign_targets(self, input_dict):
        ...
        extend_gt_boxes = x2ms_adapter.tensor_api.view(box_utils.enlarge_box3d(
            x2ms_adapter.tensor_api.view(gt_boxes, -1, gt_boxes.shape[-1]), extra_width=self.model_cfg.TARGET_CONFIG.GT_EXTRA_WIDTH
    ...
    def construct(self, batch_dict):
        ...
        point_cls_scores = x2ms_adapter.sigmoid(point_cls_preds)
        point_part_offset = x2ms_adapter.sigmoid(point_part_preds)
        batch_dict['point_cls_scores'], _ = x2ms_adapter.tensor_api.x2ms_max(point_cls_scores, dim=-1)
