...
def cfg_from_list(cfg_list, config):
    ...
    for k, v in zip(cfg_list[0::2], cfg_list[1::2]):
        key_list = x2ms_adapter.tensor_api.split(k, '.')
        d = config
        ...
        if type(value) != type(d[subkey]) and isinstance(d[subkey], EasyDict):
            key_val_list = x2ms_adapter.tensor_api.split(value, ',')
            for src in key_val_list:
                cur_key, cur_val = x2ms_adapter.tensor_api.split(src, ':')
            ...
            val_list = x2ms_adapter.tensor_api.split(value, ',')
