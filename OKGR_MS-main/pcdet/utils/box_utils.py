...
def boxes_to_corners_3d(boxes3d):
    ...
    corners3d = x2ms_adapter.tensor_api.view(common_utils.rotate_points_along_z(x2ms_adapter.tensor_api.view(corners3d, -1, 8, 3), boxes3d[:, 6]), -1, 8, 3)
...
def corners_rect_to_camera(corners):
    ...
    for index_h, index_w, index_l, index_v in zip(height_group, width_group, length_group, vector_group):
        height += x2ms_adapter.tensor_api.norm(np.linalg, corners[index_h[0], :] - corners[index_h[1], :])
        width += x2ms_adapter.tensor_api.norm(np.linalg, corners[index_w[0], :] - corners[index_w[1], :])
        length += x2ms_adapter.tensor_api.norm(np.linalg, corners[index_l[0], :] - corners[index_l[1], :])
    ...
    center_point = x2ms_adapter.tensor_api.x2ms_mean(corners, axis=0)
    center_point[1] += height/2
    camera_rect = x2ms_np.concatenate([center_point, np.array([length, height, width, rotation_y])])
...
def mask_boxes_outside_range_numpy(boxes, limit_range, min_num_corners=1, use_center_to_filter=True):
    ...
    if use_center_to_filter:
        ...
        mask = x2ms_adapter.tensor_api.x2ms_all(((box_centers >= limit_range[0:3]) & (box_centers <= limit_range[3:6])), axis=-1)
    else:
        corners = boxes_to_corners_3d(boxes)  # (N, 8, 3)
        corners = corners[:, :, 0:2]
        mask = x2ms_adapter.tensor_api.x2ms_all(((corners >= limit_range[0:2]) & (corners <= limit_range[3:5])), axis=2)
        mask = x2ms_adapter.tensor_api.x2ms_sum(mask, axis=1) >= min_num_corners  # (N)
...
def remove_points_in_boxes3d(points, boxes3d):
    ...
    return x2ms_adapter.tensor_api.numpy(points) if is_numpy else points
...
def boxes3d_kitti_camera_to_lidar(boxes3d_camera, calib):
    ...
    xyz_lidar[:, 2] += h[:, 0] / 2
    return x2ms_np.concatenate([xyz_lidar, l, w, h, -(r + np.pi / 2)], axis=-1)
...
def boxes3d_kitti_fakelidar_to_lidar(boxes3d_lidar):
    ...
    return x2ms_np.concatenate([boxes3d_lidar_copy[:, 0:3], l, w, h, -(r + np.pi / 2)], axis=-1)
...
def boxes3d_kitti_lidar_to_fakelidar(boxes3d_lidar):
    ...
    return x2ms_np.concatenate([boxes3d_lidar_copy[:, 0:3], dy, dx, dz, -heading - np.pi / 2], axis=-1)
...
def enlarge_box3d(boxes3d, extra_width=(0, 0, 0)):
    ...
    """
    boxes3d, is_numpy = common_utils.check_numpy_to_torch(boxes3d)
    large_boxes3d = x2ms_adapter.tensor_api.clone(boxes3d)

    large_boxes3d[:, 3:6] += x2ms_adapter.tensor_api.new_tensor(boxes3d, extra_width)[None, :]
...
def boxes3d_lidar_to_kitti_camera(boxes3d_lidar, calib):
    ...
    r = -r - np.pi / 2
    return x2ms_np.concatenate([xyz_cam, l, h, w, r], axis=-1)
...
def boxes3d_to_corners3d_kitti_camera(boxes3d, bottom_center=True):
    ...
    if bottom_center:
        ...
        y_corners[:, 4:8] = -x2ms_adapter.tensor_api.repeat(h.reshape(boxes_num, 1), 4, axis=1)  # (N, 8)
    else:
        y_corners = np.array([h / 2., h / 2., h / 2., h / 2., -h / 2., -h / 2., -h / 2., -h / 2.], dtype=np.float32).T

    ry = boxes3d[:, 6]
    zeros, ones = np.zeros(ry.size, dtype=np.float32), np.ones(ry.size, dtype=np.float32)
    rot_list = np.array([[x2ms_adapter.tensor_api.cos(np, ry), zeros, -x2ms_adapter.tensor_api.sin(np, ry)],
                         [zeros, ones, zeros],
                         [x2ms_adapter.tensor_api.sin(np, ry), zeros, x2ms_adapter.tensor_api.cos(np, ry)]])  # (3, 3, N)
...
def boxes3d_kitti_camera_to_imageboxes(boxes3d, calib, image_shape=None):
    ...
    min_uv = x2ms_adapter.tensor_api.x2ms_min(np, corners_in_image, axis=1)  # (N, 2)
    max_uv = x2ms_adapter.tensor_api.x2ms_max(np, corners_in_image, axis=1)  # (N, 2)
    boxes2d_image = x2ms_np.concatenate([min_uv, max_uv], axis=1)
...
def boxes_iou_normal(boxes_a, boxes_b):
    ...
    x_min = x2ms_adapter.x2ms_max(boxes_a[:, 0, None], boxes_b[None, :, 0])
    x_max = x2ms_adapter.x2ms_min(boxes_a[:, 2, None], boxes_b[None, :, 2])
    y_min = x2ms_adapter.x2ms_max(boxes_a[:, 1, None], boxes_b[None, :, 1])
    y_max = x2ms_adapter.x2ms_min(boxes_a[:, 3, None], boxes_b[None, :, 3])
    # x_len = torch.clamp_min(x_max - x_min, min=0)
    # y_len = torch.clamp_min(y_max - y_min, min=0)
    x_len = mindspore.ops.clamp(x_max - x_min, min=0)
    y_len = mindspore.ops.clamp(y_max - y_min, min=0)
    ...
    iou = a_intersect_b / mindspore.ops.clamp(area_a[:, None] + area_b[None, :] - a_intersect_b, min=1e-6)
    return iou
...
def boxes3d_lidar_to_aligned_bev_boxes(boxes3d):
    ...
    choose_dims = x2ms_adapter.where(rot_angle[:, None] < np.pi / 4, boxes3d[:, [3, 4]], boxes3d[:, [4, 3]])
    aligned_bev_boxes = x2ms_adapter.cat((boxes3d[:, 0:2] - choose_dims / 2, boxes3d[:, 0:2] + choose_dims / 2), dim=1)
...
def pairwise_iou(boxes1, boxes2) -> mindspore.Tensor:
    """
    ...
    width_height = x2ms_adapter.x2ms_min(boxes1[:, None, 2:], boxes2[:, 2:]) - x2ms_adapter.x2ms_max(
        boxes1[:, None, :2], boxes2[:, :2]
    )  # [N,M,2]

    x2ms_adapter.tensor_api.clamp_(width_height, min=0)  # [N,M,2]
    inter = x2ms_adapter.tensor_api.prod(width_height, dim=2)  # [N,M]
    del width_height

    # handle empty boxes
    iou = x2ms_adapter.where(
        inter > 0,
        inter / (area1[:, None] + area2 - inter),
        x2ms_adapter.zeros(1, dtype=inter.dtype, device=inter.device),
    )
    return iou
